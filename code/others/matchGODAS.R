...
matchGODAS <- function(data, lonlat_cols, date_col,
                       var_label = 'env_var', var_path,
                       depth_range = NULL,
                       depth_FUN = "mean",
                       na_rm = TRUE,
  ...
  for(i in seq_along(monthList)){
    ...
    start_day = lubridate::floor_date(monthList[i], "year")
    ...
    index = which(these_nctimes == as.POSIXct(monthList[i], tz = 'UTC'))
